module.exports = (async(self,local)=>{
    ...
    if(c.first().speaking.has("SPEAKING")) return local.message.reply("Player is not paused.");
    if(!c.first().dispatcher.paused) return local.message.reply("Player is not paused.");
    c.first().dispatcher.resume();
    return local.message.reply(":play_pause: **Resumed**");
})