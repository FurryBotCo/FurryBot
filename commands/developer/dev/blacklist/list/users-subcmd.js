...
module.exports = {
	...
	botPermissions: [],
	cooldown: 0,
	description: "Lists blacklisted userss",
	usage: "<page>",
	...
	run: (async function(message) {
		...
		let entries = await mdb.collection("users").find({ blacklisted: true }).toArray();
		...
		for(let en of entries) {
			let s = await this.bot.getRESTUser(en.id);

			if(!s) e.push(`\`${en.id}\` - ${en.blacklistReason}`);
			else e.push(`\`${s.username}#${s.discriminator}\` (\`${en.id}\`) - ${en.blacklistReason}`);
		}
