...
module.exports = {
	...
	run: (async function(message) {
		...
		for(let p in config.Permissions.constant) {
			if(message.member.permission.allow & config.Permissions.constant[p]) allow_user.push(p);
			else deny_user.push(p);
		}

		for(let p in config.Permissions.constant) {
			if(b.allow & config.Permissions.constant[p]) allow_bot.push(p);
			else deny_bot.push(p);
		}

		au = allow_user.length === 0 ? "NONE" : allow_user.join("**, **"),
		du = deny_user.length === Object.keys(config.Permissions.constant).length ? "NONE" : deny_user.join("**, **"),
		ab = allow_bot.length === 0 ? "NONE" : allow_bot.join("**, **"),
		db = deny_bot.length === Object.keys(config.Permissions.constant).length ? "NONE" : deny_bot.join("**, **");
		embed = {
