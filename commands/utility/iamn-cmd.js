...
module.exports = {
	...
	run: (async function(message) {
		...
		if(!role || role.length === 0) return message.channel.createMessage("Role not found.");
		role = role[0];
		if(!message.member.roles.includes(role.id)) return message.channel.createMessage("You don't have this role.");
		...
		await message.member.removeRole(role.id,"iamnot command");
		return message.channel.createMessage(`<@!${message.author.id}>, You no longer have the **${role.name}** role.`);
	})
};