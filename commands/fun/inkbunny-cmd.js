...
module.exports = {
	...
	run: (async function(message) {
		...
		if(!config.apis.inkbunny.sid) {
			await this.fsn.readFile(`${process.cwd()}/inkbunny-sid.txt`,"UTF8").then(async(sid) => {
				...
				if(typeof a.error_code !=="undefined" && typeof a.sid === "undefined") {
					switch(a.error_code) {
					case 2:
						login = await this.request(`https://inkbunny.net/api_login.php?${config.apis.inkbunny.urlCredentials}`,{
							method: "GET",
							headers: {
								"User-Agent": config.web.userAgent
							}
				...
				if(e.code === "ENOENT") {
					a = await this.request(`https://inkbunny.net/api_login.php?${config.apis.inkbunny.urlCredentials}`,{
						method: "GET",
						headers: {
							"User-Agent": config.web.userAgent
						}
			...
			req = await this.request(`https://inkbunny.net/api_userrating.php?sid=${config.apis.inkbunny.sid}`,{
				method: "GET",
				headers: {
					"User-Agent": config.web.userAgent
				}
		...
		req = await this.request(`https://inkbunny.net/api_search.php?sid=${config.apis.inkbunny.sid}&orderby=views&type=1,3,5,8,9&count_limit=50000&submissions_per_page=100&text=${tags}&random=yes&get_rid=yes`,{
			method: "GET",
			headers: {
				"User-Agent": config.web.userAgent
			}
