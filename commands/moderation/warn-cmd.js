...
module.exports = {
	...
	run: (async function(message) {
		...
		w = await mdb.collection("users").findOneAndUpdate({id: user.id},{$push: {warnings: {wid: u.warnings.length + 1, blame: message.author.id, reason, timestamp: Date.now(), gid: message.channel.guild.id}}});
