module.exports = {
	...
	run: (async function(message) {
        ...
        var warnings = await this.db.getUserWarnings(user.id,message.guild.id).then(res=>res.sort((a,b)=>new Date(a.timestamp) - new Date(b.timestamp)));
        ...
        for(let key in wr[page-1]) {
            var w = wr[page-1][key];
            var usr = await this.users.fetch(w.blame);
