module.exports=(async (self,local) => {
	...
	if(!local.args[0]) {
		var lnk=local.gConfig.prefix !== "f!"?`${self.config.documentationURL}?prefix=${local.gConfig.prefix}${self.config.beta?"&beta":""}`:`${config.documentationURL}${self.config.beta?"?beta":""}`;
