module.exports = (async function(message){
    ...
    try {
        Object.assign(message,{
            ...
            gConfig: await this.db.getGuild(message.guild.id).catch(err=>this.config.default.guildConfig),
            uConfig: await this.db.getUser(message.author.id).catch(err=>this.config.default.userConfig),
            get prefix() {
                return message.content.startsWith(`<@${this.user.id}>`) ? `<@${this.user.id}` : message.content.startsWith(`<@!${this.user.id}>`) ? `<@!${this.user.id}>` : this.config.beta || this.config.alpha ? this.config.defaultPrefix : message.gConfig.prefix.toLowerCase();
            },
            get args() {
                try {
	...
	if(message.content === `<@${this.user.id}>` || message.content === `<@!${this.user.id}>` && !blacklist) {
		/*var c = await require(`${process.cwd()}/commands/${this.config.commandList.fullList["help"].category}/help-cmd.js`)(this,message);
	...
	if(this.responseList.includes(message.content.toLowerCase()) && !blacklist) {
		var response = this.getResponse(message.content.toLowerCase());
		...
		var c = await response.run(this,message);
    ...
    var command = this.getCommand(message.command);
    var category = this.getCategory(message.command);

	if(blacklist) {
		var g = await this.db.isBlacklisted(message.guild.id);
		var u = await this.db.isBlacklisted(message.author.id);
    ...
    try {
		...
		var start = this.performance.now();
		var c = await command.run(message);
		var end = this.performance.now();
        ...
        if(e.message === "ERR_INVALID_USAGE") {
			...
			this.logger.error(`[CommandHandler] e1: ${e.name}: ${e.message}\n${e.stack}`);
			this.analytics.track({
				userId: "CLIENT",
				event: `client.errors`,
				properties: {
