module.exports = (async function(guild,user) {
    ...
    this.analytics.track({
        userId: "CLIENT",
        event: "client.events.guildBanAdd",
        properties: {
            bot: {
    ...
    var data = {
        title: `${guild.me.permissions.has("USE_EXTERNAL_EMOJIS") ? this.config.emojis.abanned : ":hammer"} Member Banned`,
        author: {
    ...
    // audit log check
    var log = await this.getLogs(guild.id,"MEMBER_BAN_ADD",user.id);
    if(log !== false) {
         data.fields.push({
            name: "Executor",
