class FurryBotDatabase {
	...
	async createGuild(gid,disableCheck=false){
		...
		await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.guilds).insert(Object.assign({id:gid},this.config.default.guildConfig));
	...
	async deleteGuild(gid) {
		...
		var b = await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.guilds).get(gid).delete();
	...
	async updateGuild(gid,fields) {
		...
		await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.guilds).get(gid).update(fields);
		return this.getGuild(gid);
	}
	...
	async sweepGuilds(del=false) {
		...
		var g = [];
		this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.guilds).then(j=>j.map(g=>g.id)).then((a) => {
			a.forEach((b)=>{
		...
		g.forEach((guild)=>{
			if(!this.client.guilds.includes(guild)) {
				...
				if(del) this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.guilds).get(guild).delete().then((r)=>{
	...
	async createUserWarning(uid,gid,blame,reason,bypassChecks=false) {
		...
		if(!(await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.users).get(uid))) await this.createUser(uid,gid);
		var wid = (await this.getUserWarnings(uid,gid)).length+1;
		if(isNaN(wid)) wid = 1;
		await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.users).get(uid).update({warnings:this.client.r.row("warnings").append({wid,blame,reason,timestamp:Date.now(),gid})});
	...
	async clearUserWarnings(uid,gid) {
		...
		var j = await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.users).get(uid).update((row)=>{
			return {
	...
	async addDonator(uid,level) {
		...
		return (await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.users).get(uid).update({donator:true,level})).replaced >= 1;
	}
	...
	async removeDonor(uid) {
		...
		var b = await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.users).getAll(uid);
		if(b.length<=0) await this.createUser(uid);
		return (await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.users).get(uid).update({donator:false,level:0})).replaced >= 1;
	}
	...
	async addBlacklistEntry(id,type = "user",reason = "None Specified") {
		...
		var c = await this.client.r.db(this.config.db.databases.main).table(table).getAll(uid);
		if(c.length<=0) await this[`create${type}`](id);
		return (await this.client.r.db(this.config.db.databases.main).table(table).get(id).update({blacklisted:true,blacklistReason:reason})).replaced >= 1;
	}
	...
	async removeBlacklistEntry(id,type = "user") {
		...
		var c = await this.client.r.db(this.config.db.databases.main).table(table).getAll(uid);
		if(c.length<=0) await this[`create${type}`](id);
		return (await this.client.r.db(this.config.db.databases.main).table(table).get(id.replace(r.row.without("blacklisted","blacklistReason").merge({blacklisted:false})))).replaced >= 1;
	}
	...
	async getStats(type) {
		...
		if(![undefined,null,""].includes(type)) {
			...
			switch(type.toLowerCase()) {
				case "fcount":
				case "f":
					return (await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.stats).get("fCount")).count;
					break;

				case "commands":
					return this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.stats).get("commands");
					break;

				case "general":
					return this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.stats).get("general");
					break;

				case "dailyjoins":
					return (await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.daily)).map(day=>({[day.id]:day.count}));
					break;
					
				case "messagecount":
					return (await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.stats).get("messageCount")).count;
					break;
			}
		} else {
			return {
				fCount: (await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.stats).get("fCount")).count,
				commands: await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.stats).get("commands"),
				general: await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.stats).get("general"),
				dailyjoins: (await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.daily)).map(day=>({[day.id]:day.count})),
				messageCount: (await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.stats).get("messageCount")).count
			};
		}
	}
	...
	async incrementCommandStats(command,amount=1) {
		...
		if(!a[command]) {
			await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.stats).get("commands").update({[command]:amount});
			return amount;
		} else {
			await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.stats).get("commands").update({[command]:+a[command]+amount});
	...
	async updateDailyCount(negative = false, amount = 1) {
		...
		var j = await this.client.r.table(this.config.db.tables.daily).getAll(date);
		if(j.length < 1) var j = await this.client.r.table(this.config.db.tables.daily).insert({id:date,count:0}).then(s=>this.client.r.table("dailyjoins").get(date));
		if(j[0]) j = j[0];
		var res = negative ? await this.client.r.table(this.config.db.tables.daily).get(date).update({count:+j.count-amount}) : await this.client.r.table("dailyjoins").get(date).update({count:+j.count+amount});
		return (await this.client.r.table(this.config.db.tables.daily).get(date)).count;
	}
	...
	async createUser(uid,bypassChecks=false) {
		...
		await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.users).insert(Object.assign({id:uid},this.config.default.userConfig));
	...
	async getUser(uid) {
		...
		return await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.users).get(uid);
	}
	...
	async deleteUser(uid) {
		...
		return (await this.client.r.db(this.config.db.databases.main).table("users").get(uid).delete()).deleted>=1;
	}
	...
	async updateUser(uid,fields) {
		...
		await this.client.r.db(this.config.db.databases.main).table(this.config.db.tables.users).get(uid).update(fields);
		return this.getUser(uid);
	}
}
