...
class ExtendedMessage extends Message {
    ...
    uConfig: UserConfig;
    gConfig: GuildConfig;
    constructor(msg: Message, client: FurryBot) {
    ...
    async _load() {
        if (!this.channel.guild) {
            ...
            this.uConfig = await mdb.collection("users").findOne({ id: this.author.id }).then(async (res) => {
                if (!res) {
                    await mdb.collection("users").insertOne(Object.assign({ id: this.author.id }, config.defaults.userConfig)).catch(err => null);
            ...
            this.gConfig = await mdb.collection("guilds").findOne({ id: this.channel.guild.id }).then(async (res) => {
                if (!res) {
                    await mdb.collection("guilds").insertOne(Object.assign({ id: this.channel.guild.id }, config.defaults.guildConfig)).catch(err => null);
        ...
        try {
            ...
            let a = this.content.slice(this.prefix.length).trim().split(/\s+/);
            a.shift();
            try {
    ...
    /*get gConfig(): GuildConfig {
        return mdb.collection("guilds").findOne({ id: this.channel.guild.id }).then(async (res) => {
            if (!res) {
                await mdb.collection("guilds").insertOne(Object.assign({ id: this.channel.guild.id }, config.defaults.guildConfig));
                console.debug(`Created Guild Entry "${this.channel.guild.id}`);
    ...
    get uConfig(): UserConfig {
        return mdb.collection("users").findOne({ id: this.author.id }).then(async (res) => {
            if (!res) {
                await mdb.collection("users").insertOne(Object.assign({ id: this.author.id }, config.defaults.userConfig));
                console.debug(`Created User Entry "${this.author.id}`);
    ...
    async getUserFromArgs(argPosition: number = 0, unparsed: boolean = false, join: boolean = false, mentionPosition: number = 0): Promise<Eris.User> {
        const {
            User,
            Member,
            Message,
    ...
    async getMemberFromArgs(argPosition: number = 0, unparsed: boolean = false, join: boolean = false, mentionPosition: number = 0): Promise<Eris.Member> {
        const {
            User,
            Member,
            Message,
    ...
    async getChannelFromArgs(argPosition: number = 0, unparsed: boolean = false, join: boolean = false, mentionPosition: number = 0): Promise<Eris.AnyGuildChannel> {
        const {
            User,
            Member,
            Message,
    ...
    async getRoleFromArgs(argPosition: number = 0, unparsed: boolean = false, join: boolean = false): Promise<Eris.Role> {
        const {
            User,
            Member,
            Message,
    ...
    async getServerFromArgs(argPosition: number = 0, unparsed: boolean = false, join: boolean = false): Promise<Eris.Guild> {
        const {
            User,
            Member,
            Message,
    ...
    async configureUser(user = null): Promise<{
        ...
        let member = !user ? user instanceof User ? this.guild.members.get(user.id) : user instanceof Member ? user : !isNaN(user) ? this.guild.members.get(user) : false : this.member;
    ...
    async errorEmbed(type: string = "", custom: boolean = false, title: string = "", description: string = "", fields: any[] = [], color: number = functions.randomColor()): Promise<Eris.Message> {
        if (!custom) {
