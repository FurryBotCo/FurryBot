...
export default {
	handleImage: (async function (client: FurryBot, msg: ExtendedMessage, path: string, extra?: { avatars?: string[]; usernames?: string[]; text?: string }) {
		...
		const test = await client.f.validateURL(imgurl);
		...
		const req = await client.f.memeRequest(`/${path}`, [imgurl, ...(extra && extra.avatars && extra.avatars.length > 0 ? extra.avatars : [])], extra && extra.usernames && extra.usernames.length > 0 ? extra.usernames : [], extra && extra.text ? extra.text : null);
