...
export default new ClientEvent<FurryBot>("channelDelete", async function (channel) {
	...
	if (channel instanceof Eris.GuildChannel) {
		...
		const events = g.logEvents.filter(l => l.type === "channelDelete");
		...
		for (const event of events) {
			...
			if (guild.permissionsOf(this.bot.user.id).has("viewAuditLog")) {
				const { entries: a } = await guild.getAuditLog({
					limit: 10,
					actionType: Eris.Constants.AuditLogActions.CHANNEL_DELETE
				});
