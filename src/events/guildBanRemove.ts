...
export default new ClientEvent<FurryBot>("guildBanRemove", async function (guild, user) {
	if (config.beta && !config.eventTest) return;
	const g = await db.getGuild(guild.id).then(v => v.fix());
	const events = g.logEvents.filter(l => l.type === "memberUnban");
	...
	for (const event of events) {
		...
		if (guild.permissionsOf(this.bot.user.id).has("viewAuditLogs")) {
			const { entries: a } = await guild.getAuditLog({
				limit: 10,
				actionType: Eris.Constants.AuditLogActions.MEMBER_BAN_REMOVE
			});
