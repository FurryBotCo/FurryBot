...
export default new ClientEvent<FurryBot>("voiceChannelSwitch", async function (member, channel, oldChannel) {
	...
	for (const event of events) {
		const ch = member.guild.channels.get(event.channel) as Eris.GuildTextableChannel;
		...
		const e = new EmbedBuilder(g.settings.lang)
			...
			.setDescription([
				...
				`{lang:other.words.channel$ucwords$}: **${oldChannel.name} -> ${channel.name}**`
			].join("\n"));
