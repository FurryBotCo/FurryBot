...
export default new Command(["kick"], __filename)
	...
	.setExecutor(async function (msg, cmd) {
		...
		if ((c.member2.higher || c.member2.same) && msg.author.id !== msg.channel.guild.ownerID) return msg.reply(Language.get(msg.gConfig.settings.lang, `${cmd.lang}.noKickOther`, [`${member.user.username}#${member.user.discriminator}`]));
		...
		await msg.channel.guild.kickMember(member.id, `Kick: ${msg.author.username}#${msg.author.discriminator} -> ${reason}`).then(async () => {
			...
			await this.m.createKickEntry(msg.channel, msg.gConfig, msg.author, member);
		}).catch(async (err) => {
