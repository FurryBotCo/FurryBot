...
export default fs.readdirSync(__dirname).filter(d => fs.lstatSync(`${__dirname}/${d}`).isDirectory()).map(f => {
	...
	const cmd = fs.readdirSync(`${__dirname}/${f}`).filter(d => (d.endsWith(".js") || d.endsWith(".ts")) && !fs.lstatSync(`${__dirname}/${f}/${d}`).isDirectory() && !d.match(/index\.(t|j)s/)).map(j => require(`${__dirname}/${f}/${j}`).default);
