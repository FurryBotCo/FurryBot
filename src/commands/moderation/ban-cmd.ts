...
export default new Command(["ban"], __filename)
	...
	.setExecutor(async function (msg, cmd) {
		...
		if (msg.channel.guild.members.has(user.id)) {
			...
			if ((c.member2.higher || c.member1.same) && msg.author.id !== msg.channel.guild.ownerID) return msg.reply(Language.get(msg.gConfig.settings.lang, `${cmd.lang}.noBanOther`, [`${user.username}#${user.discriminator}`]));
