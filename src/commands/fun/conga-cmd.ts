...
export default new Command({
	...
	if (h && msg.args.length === 0) {
		...
		return msg.channel.createMessage(`{lang:commands.fun.conga.join|${msg.author.id}|${c.length + 1}|${gConfig.settings.prefix}|${c.length + 1}|${c.length + 1 > 30 ? Language.parseString(gConfig.settings.lang, "{lang:commands.fun.conga.tooLarge}") : `<a:${config.emojis.conga}>`.repeat(c.length + 1)}}`);
	} else {
