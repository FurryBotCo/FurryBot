...
export default new Command({
	...
	if (h && msg.args.length === 0) {
		...
		return msg.channel.createMessage(`{lang:commands.fun.conga.join|${msg.author.id}|${c.length + 1}|${gConfig.settings.prefix}|${c.length + 1}|${c.length + 1 > 30 ? Language.get(gConfig.settings.lang, "commands.fun.conga.tooLarge") : config.emojis.conga.repeat(c.length + 1)}}`);
	} else {
		...
		this.holder.set("conga", `${msg.channel.id}.timeout`, setTimeout((ch: Eris.GuildTextableChannel) => { this.holder.remove("conga", ch.id); this.holder.remove("conga", `${ch.id}.timeout`); }, 18e5, msg.channel));
		await msg.channel.createMessage(`{lang:commands.fun.conga.start|${msg.author.id}|${m.id}|${gConfig.settings.prefix}|${config.emojis.conga}}`);
	}
}));
