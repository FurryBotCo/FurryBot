	...
	return msg.channel.createMessage({
		embed: new EmbedBuilder(gConfig.settings.lang)
			...
			.setDescription([
				...
				`\u25FD {lang:other.words.shard}: ${msg.channel.guild.shard.id + 1}/${this.shards.size}`,
				`\u25FD {lang:other.words.guilds}: ${this.guilds.size}`,
				`\u25FD {lang:commands.information.info.largeGuilds}: ${this.guilds.filter(g => g.large).length}`,
				`\u25FD {lang:other.words.users}: ${this.users.size}`,
				`\u25FD {lang:other.words.channels}: ${Object.keys(this.channelGuildMap).length}`,
				`\u25FD {lang:commands.information.info.voiceConnections}: ${this.voiceConnections.size}`,
