...
export default new Command({
	...
	if (!perm.some(p => msg.channel.permissionsOf(this.user.id).has(p))) {
		...
		else if (e[0].file_ext === "swf") embed.description = `This post is a flash animation, please directly view [the post](https://e621.net/post/show/${e[0].id}) on e621`;
		else embed.description = `This post appears to be a video, please directly view [the post](https://e621.net/post/show/${e[0].id}) on e621`;
	...
	try {
		...
		if (e.message === "ERR_MISSING_PERMISSIONS") return msg.reply("I seem to be missing some permissions. Please try again, and pay attention to any permission warnings!");
	}
}));
