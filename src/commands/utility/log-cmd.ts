...
import { BotFunctions, Colors, Command, CommandError, EmbedBuilder, ErisPermissions } from "core";
...
export default new Command<FurryBot, UserConfig, GuildConfig>(["log"], __filename)
	...
	.setExecutor(async function (msg, cmd) {
		...
		function f(t: string) {
			...
			return config.logTypes.find(v => v.toLowerCase() === t.toLowerCase());
		}
		switch (msg.args[0]?.toLowerCase()) {
			case "add": {
				if (msg.gConfig.logEvents.length >= max) return msg.reply(Language.get(msg.gConfig.settings.lang, `${cmd.lang}.add.max`, [max, msg.prefix]));
				...
				if (t === "all") {
					await msg.gConfig.mongoEdit({
						$push: {
							logEvents: {
								$each: config.logTypes.map(v => ({
									channel: ch.id,
									type: v,
									ignore: []
								})) as unknown as GuildConfig["logEvents"]
							}
						}
					});
			...
			case "available": {
				...
				const text = ["all", ...config.logTypes].map(v => `**${v}** - ${Language.get(msg.gConfig.settings.lang, `${cmd.lang}.available.${v}`)}`);
