...
export default new Command({
	...
	if (count < 2 || count > 100) return msg.reply("{lang:commands.utility.purge.amount}");
	if (count < 100) count++;
	...
	if (m.length !== f.length) await msg.channel.createMessage(`{lang:commands.utility.purge.skipped|${m.length - f.length}}`);
}));
