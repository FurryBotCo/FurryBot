...
export default new Command({
	...
	if ([`<@!${this.bot.user.id}>`, `<@${this.bot.user.id}>`].some(t => msg.args.join("").toLowerCase() === t.toLowerCase())) return msg.reply(`{lang:commands.utility.prefix.invalid|${msg.args.join("").toLowerCase()}}`);
